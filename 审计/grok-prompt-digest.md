# 任务
你是独立审计方。禁止调用工具。根据下列摘录写完整中文审计报告。不要说总体很棒。

产品：美国英语 SEO 工具站，今晚某地极光值不值得出门。无登录/App/订阅。获客是搜索。钱页 /forecast/[slug]。首页 SSR 不得按 IP 改成某城。Wave 1 = 15 个美国地点。下一步计划：先引擎快照，HTML 等线框过了再做。

# 要审
A 文档打架 B SEO获客 C 引擎 D 地点范围 E 线框vs页面 F 开工缺口
...
---
# 线框要点（非全文）
首页：H1 泛词 + 搜索 + Tonight in the US 全量 15 行表（状态/地点/窗口/Open）+ What time + How to read。无 IP 结论卡。
地点页：H1 问句 + 答案段 + 结论卡 Share + 小时轴 + Why + What to do + 州内代表点 + FAQ；桌面右栏 Nearby。
Alaska：今晚卡后 When to come / Which part。
near-me：独立 H1，提交后跳走。
/view：noindex 结论卡。
Find place 层：ZIP/城市/GPS，命中白名单跳 forecast。
无地图、无登录。手机地点第一屏：H1+答案段+卡。

---
# 档案摘要
15 slug 均 seo_indexable=false。州页有 2-3 sample_points + primary_verdict_point（colorado=fort-collins, ohio=cleveland, illinois=rockford, minnesota=duluth, massachusetts=newburyport, oregon=portland, alaska=fairbanks）。
chicago parent=illinois; fairbanks parent=alaska; seattle parent=null, nearby 仅 oregon。
吸收：Boston→massachusetts, Northern Michigan→michigan, Minneapolis/Duluth→minnesota。
typical_kp_horizon: fairbanks 1, minnesota 4, seattle 5, chicago/ohio 6, colorado/utah 7。偶极磁纬，非 AACGM。
oregon variants 仅 2 条。colorado nearby 仅 utah。

---
# PRD 对照（非 v1 正文）
首发 16 城；URL 含 /map P1、/places/alaska；判断 45/30/15/10 加权、70+GO、未入夜 WAIT；首页须覆盖地图；实时数据须在 SSR HTML。
