...
- Keep `engine/snapshot.py`; add a GitHub Action every **10 minutes** that always commits if `generated_at` changed. `valid_until` is `generated_at + 25 minutes` (capped at OVATION forecast + 40 minutes).
...
- Unknown `/forecast/[slug]` → 404. Alias Boston→massachusetts etc. only on Find place / near-me submit.
